...
test('search for insights containing searchTerm in title', function (t) {
  init(config, function (err, server, pool) {
    server.inject({ url: '/insightsSearch?searchTerm=insight number 1' }, function (res) {
...
test('search for insights containing searchTerm in author', function (t) {
  init(config, function (err, server, pool) {
    server.inject({ url: '/insightsSearch?searchTerm=jones' }, function (res) {
...
test('search for insights containing searchTerm in tags', function (t) {
  init(config, function (err, server, pool) {
    server.inject({ url: '/insightsSearch?searchTerm=agriculture' }, function (res) {
