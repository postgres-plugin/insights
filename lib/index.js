...
function register (server, options, next) {
  ...
  var getMatchingInsights = function (listOfTags, cb) {
    if (listOfTags.length === 0) {
      return cb(null, []);
    }

    query(queries.getMatchingInsights(listOfTags), pool, function (queryErr, res) {
      if (queryErr) {
        return cb(queryErr);
      }

      return cb(null, formatters.getMatchingInsights(res.rows));
    });
  };
  ...
  query(initQuery, pool, function (initErr) {
    ...
    server.method('pg.insights.getMatchingInsights', getMatchingInsights);
